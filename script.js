...
blockes = Array.from(document.getElementsByClassName('a'));

blockes.forEach(block => {
    block.addEventListener('click', firstClicked);
});
...
function firstClicked(block) {
    let clickedPice = getPiecePresent(block.target.id);
    if (clickedPice == undefined) {
        clickedPice = "undefine";
        return;
    }
    if (clickedPice.slice(0, 1) == turn) {
        firstPice = clickedPice.slice(0, 1);

        blockes.forEach(block => {
            block.removeEventListener('click', firstClicked);
            block.addEventListener('click', secondClicked);
        });
...
function secondClicked(block) {
    ...
    let status = sameColorCheck(firstBlockClicked, secondBlockClicked);
    if (!status) {
        status = checkMoves(firstBlockClicked, secondBlockClicked);
        if (status) {
            status = checkInBetween(firstBlockClicked, secondBlockClicked);
            if (!status) {

                movePieces(firstBlockClicked, secondBlockClicked);
                blockes.forEach(block => {
                    block.addEventListener('click', firstClicked);
                    block.removeEventListener('click', secondClicked);
                });
                changeTurn(turn);
...
function checkMoves(firstBlockClicked, secondBlockClicked) {
    ...
    if (piecesOnFisrt == null) {
        ...
        if (e == (s + 10) || e == (s + 20) || e == (s + 30) || e == (s + 40) || e == (s + 50) || e == (s + 60) || e == (s + 70)) {
            if (s.toString().slice(1, 2) == e.toString().slice(1, 2))
                return true;
        }
            ...
            if (s.toString().slice(0, 1) == e.toString().slice(0, 1)) {
                return true;
            };
        }
        else if (e == (s - 1) || e == (s - 2) || e == (s - 3) || e == (s - 4) || e == (s - 5) || e == (s - 6) || e == (s - 7)) {
...
function checkInBetween(firstPosition, secondPosition) {

    let s = parseInt(firstPosition), e = parseInt(secondPosition);
    if (getPiecePresent(firstPosition) == "blackBishop" || getPiecePresent(firstPosition) == "whiteBishop") {
        return (inbetweenBishop(s, e));
    }
    else if (getPiecePresent(firstPosition) == "blackRook" || getPiecePresent(firstPosition) == "whiteRook") {
        return (inbetweenRook(s, e));
    }
    else if (getPiecePresent(firstPosition) == "blackQueen" || getPiecePresent(firstPosition) == "whiteQueen") {
        return (inbetweenQueen(s, e));
    }
}
...
function inbetweenQueen(s, e) {           // its a fuckallllll solution plz dont judge me with this lines of code ;D i also dont want to write this
    // top right
    if (e == s + 11 || e == s + 22 || e == s + 33 || e == s + 44 || e == s + 55 || e == s + 66 || e == s + 77) {
        s = s + 11;
        if (s == e) { return false }
